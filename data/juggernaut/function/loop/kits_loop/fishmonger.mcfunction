...
execute as @a[tag=fishmonger,tag=!in_chase] if items entity @s container.* trident run scoreboard players set @s ability_cooldown1 4
execute as @a[tag=fishmonger,tag=!in_chase] run clear @a trident[damage=249]
execute as @a[tag=fishmonger,tag=!in_chase,scores={ability_cooldown1=0}] unless items entity @s container.* trident run give @s trident[item_name={"text": "Speartooth Trident"},enchantments={riptide:2},damage=248,lore=[{"text":"","color":"dark_gray"}]] 1

execute as @a[tag=fishmonger,tag=in_chase] run clear @s trident[enchantments={riptide:2}]
