...
summon armor_stand ~ ~0.5 ~ {Invisible:true,Invulnerable:true,CustomName:{"text": "Revealing Tower","bold": true,"color": "gold"},CustomNameVisible:false,Small:true,NoGravity:true,Tags:["warlock_revealing_tower","warlock_armor_stand","kill_on_end_game"]}
# Duration
...
scoreboard players set @n[type=armor_stand,tag=warlock_revealing_tower] total_dispelling_needed 300
bossbar add warlock:revealing_tower "Revealing Tower"
bossbar set warlock:revealing_tower value 0
bossbar set warlock:revealing_tower max 300
