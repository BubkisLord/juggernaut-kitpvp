...
execute as @s[tag=!has_jug_kit] run item replace entity @s hotbar.3 with bone[item_name='[{"text": "Spawn Turret","color": "gray","bold": true},{"text": " | "},{"text": "READY","color": "green"}]']
execute as @s[tag=!has_jug_kit] run item replace entity @s hotbar.4 with clock[item_name='{"text": "Borrowed Time Ability"}',lore=['{"text": "Taking damage for the next 10 seconds is nullified","color":"dark_gray"}','{"text":"but then later applied periodically in stacks of 2.","color": "dark_gray"}']]
execute as @s[tag=!has_jug_kit] run item replace entity @s hotbar.5 with golden_apple 3
execute as @s[tag=!has_jug_kit] run item replace entity @s hotbar.6 with cooked_beef 32
