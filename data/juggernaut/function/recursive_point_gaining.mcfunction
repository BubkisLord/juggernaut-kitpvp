...
execute as @e[tag=replenishment.station] at @s if entity @e[type=armor_stand,tag=juggernaut_manager,scores={game_state=1}] unless entity @e[type=armor_stand,tag=banishment_glyph,distance=..32] as @p[tag=runner,distance=0..3] at @s run scoreboard players add @s points 1
