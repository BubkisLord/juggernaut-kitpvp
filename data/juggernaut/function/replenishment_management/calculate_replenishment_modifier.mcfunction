...
# Puppeteer passive: replenishes at only 70% efficiency (perks still stack on top of this).
execute if entity @s[tag=puppeteer] run scoreboard players remove @s replenishment_modifier 30
...
execute if entity @s[tag=using_unwavering_strength] if entity @e[type=armor_stand,tag=replenishment.station,distance=..3] at @s if entity @a[tag=juggernaut,distance=..18] run scoreboard players add @s replenishment_modifier 100
...
execute as @e[type=armor_stand,tag=replenishment_tower] at @s as @a[tag=runner,distance=..3] unless entity @n[type=armor_stand,tag=replenishment.station,distance=..3] run scoreboard players add @s replenishment_modifier 20
