execute at @a[tag=warlock] run execute as @e[type=item,nbt={Item:{id:"minecraft:black_dye"}},distance=..3] run summon armor_stand ~ ~0.5 ~ {NoGravity:true,Invulnerable:true,Invisible:true,CustomName:'{"text": "Withering Surge","color": "black","bold": true}',CustomNameVisible:true,Tags:["withering_surge","warlock_armor_stand"]}
