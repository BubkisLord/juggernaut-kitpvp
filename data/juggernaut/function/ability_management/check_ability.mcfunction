...
$execute as @a[tag=$(player_tag)] at @s if entity @e[type=item,nbt={Item:{id:"$(item_id)",components:{"minecraft:custom_data":{kit:$(player_tag),ability_id:"$(ability_id)"}}}},distance=..3] if score #game_state var matches 10..12 unless score #game_state var matches 11 run particle angry_villager ~ ~ ~ 0.5 1 0.5 0 60 force
