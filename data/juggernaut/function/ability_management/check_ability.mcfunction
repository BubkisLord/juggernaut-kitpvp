...
$execute as @a[tag=$(player_tag)] if score @s $(cooldown_var) matches 0 run item replace entity @s $(hotbar_slot) with $(item_id)[item_name='[$(item_name),{"text": " | ","color": "dark_gray","bold": true},{"text": "READY","color": "green","bold": true}]']
...
$execute as @a[tag=$(player_tag)] at @s if entity @e[type=item,nbt={Item:{id:"$(item_id)"}},distance=..3] run data modify storage juggernaut:ability_management/use_ability ability_id set value $(ability_id)
$execute as @a[tag=$(player_tag)] at @s if entity @e[type=item,nbt={Item:{id:"$(item_id)"}},distance=..3] if score #game_state var matches 11 run function juggernaut:ability_management/use_ability {\
...
$execute as @a[tag=$(player_tag)] at @s if entity @e[type=item,nbt={Item:{id:"$(item_id)"}},distance=..3] run clear @s $(item_id)
$execute as @a[tag=$(player_tag)] at @s as @e[type=item,nbt={Item:{id:"$(item_id)"}},distance=..3] run kill @s