...
$execute as @a[tag=$(player_tag)] if score @s $(cooldown_var) matches 0 run item replace entity @s $(hotbar_slot) with $(item_id)[item_name=[$(item_name),{"text": " | ","color": "dark_gray","bold": true},{"text": "READY","color": "green","bold": true}],custom_data={kit:$(player_tag),ability_id:"$(ability_id)"},lore=$(description)]
$execute as @a[tag=$(player_tag)] if score @s $(cooldown_var) matches 1.. run item replace entity @s $(hotbar_slot) with barrier[item_name=[$(item_name),{"text": " | ","color": "dark_gray","bold": true},{"text": "ON COOLDOWN","color": "red","bold": true}],custom_data={kit:$(player_tag),ability_id:"$(ability_id)"},lore=$(description)]
...
$execute as @a[tag=$(player_tag)] at @s as @n[type=item,nbt={Item:{id:"$(item_id)",components:{"minecraft:custom_data":{kit:$(player_tag),ability_id:"$(ability_id)"}}}},distance=..3] at @s run function juggernaut:ability_management/check_ability_owner with storage juggernaut:ability_management/check_ability_owner
xp set @a 28 levels
