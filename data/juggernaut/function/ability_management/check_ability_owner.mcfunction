...
$execute if score #game_state var matches 41 as @p[tag=ability_owner] at @s run function juggernaut:ability_management/use_ability {\
    player_tag:$(player_tag),\
    cooldown:$(cooldown),\
    item_name:$(item_name),\
    hotbar_slot:$(hotbar_slot),\
...
$execute if score #game_state var matches 46 as @p[tag=ability_owner] at @s run function juggernaut:ability_management/use_ability {\
    player_tag:$(player_tag),\
    cooldown:$(cooldown),\
    item_name:$(item_name),\
    hotbar_slot:$(hotbar_slot),\
...
execute if score #game_state var matches 40 as @a[tag=ability_owner] at @s run particle angry_villager ~ ~ ~ 0.5 1 0.5 0 60 force @s
execute if score #game_state var matches 45 as @a[tag=ability_owner] at @s run particle angry_villager ~ ~ ~ 0.5 1 0.5 0 60 force @s
...
$execute if score #game_state var matches 40 as @a[tag=ability_owner] run scoreboard players set @s $(cooldown_var) 2
$execute if score #game_state var matches 45 as @a[tag=ability_owner] run scoreboard players set @s $(cooldown_var) 2
kill @s
