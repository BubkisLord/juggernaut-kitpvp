...
execute if entity @s[tag=teleporting] as @n[type=mannequin,tag=phase_shift_destination] at @s unless block ~ ~0.5 ~ #juggernaut:raycast_permeable run scoreboard players set @p[tag=teleporting] ability_cooldown0 0
execute if entity @s[tag=teleporting] as @n[type=mannequin,tag=phase_shift_destination] at @s if block ~ ~0.5 ~ #juggernaut:raycast_permeable run scoreboard players set @p[tag=teleporting] ability_cooldown0 3
