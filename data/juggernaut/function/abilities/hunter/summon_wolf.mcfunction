...
execute unless entity @a[tag=has_hunters_mark] run tag @r[tag=runner,tag=!has_respawn_protection] add hunted
execute if entity @a[tag=has_hunters_mark,tag=!has_respawn_protection] as @p[tag=has_hunters_mark,tag=!has_respawn_protection] run tag @s add hunted
execute unless entity @a[tag=hunted] as @a[tag=hunter] at @s run particle angry_villager ~ ~0.5 ~ 1 1 1 0 80 force
execute unless entity @a[tag=hunted] run scoreboard players set @s ability_cooldown1 8000
