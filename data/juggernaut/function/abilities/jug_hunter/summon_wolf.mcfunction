...
execute unless entity @a[tag=has_hunters_mark] run tag @r[tag=runner] add hunted
...
summon minecraft:wolf ~ ~ ~ {CustomName:{"text":"Bloodhound","color":"dark_red","italic":false},CustomNameVisible:true,attributes:[{id:"max_health",base:45},{id:"movement_speed",base:0.6},{id:"follow_range",base:700},{id:"attack_damage",base:0},{id:"scale",base:1.5}],PersistenceRequired:1b,Tags:["hunter_wolf","kill_on_end_game"],Team:"jug"}
execute as @n[type=minecraft:wolf,tag=hunter_wolf] run data modify entity @s anger_end_time set value 9223372036854775807L
execute as @a[tag=hunted,tag=has_respawn_time] run tag @r[tag=!hunted,tag=runner] add hunted
execute as @a[tag=hunted,tag=has_respawn_time] run tag @s remove hunted
