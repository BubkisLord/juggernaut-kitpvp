...
execute store result storage juggernaut:abilities/trickster/misty_step_rotation yaw float 1 run random value 1..360
...
execute as @n[type=marker,tag=misty_step_marker] at @s run summon marker ^ ^ ^12 {Tags:["misty_step_dest","kill_on_end_game"]}
