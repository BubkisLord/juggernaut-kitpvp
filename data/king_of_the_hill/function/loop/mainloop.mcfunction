...
execute if score #end_game_timer var < #0 var run function king_of_the_hill:end_game
...
# Spawn Crates
execute if score #crate_count var < #max_crate_count var run summon armor_stand 0 100 0 {Tags:["crate","kill_on_end_game"],Invisible:true,Invulnerable:true,Small:true,NoGravity:true,CustomName:{"text": "king_of_the_hill Crate","bold": true},CustomNameVisible:true}
...
execute as @e[type=armor_stand,tag=crate,scores={var=3..}] at @s run loot give @p[distance=..3,tag=!spectator] loot king_of_the_hill:crate
...
execute as @e[type=armor_stand,tag=surv_game_manager,scores={tick_counter=20..}] run function king_of_the_hill:loop/second
